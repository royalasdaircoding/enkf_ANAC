...
class EnsembleKalmanFilter(object):
    ...
    def __init__(self, x, P, dim_z, dt, N, hx, fx, inf=1, gauss_kick=None, ensemble_type="gsn", constraints=None, qscales=None, inf_a=None, seed=None,
                 n_workers = 1):
        if dim_z <= 0:
        ...
        assert type(n_workers)==int and n_workers>=1, "n_workers must be an int bigger than or equal to one"
    ...
    def update(self, z, R=None):
        ...
        self.x_prior = np.copy(self.sigmas)
        ...
        for i in range(N):
            sigmas_h[i] = self.hx(self.inf*self.sigmas[i])
        ...
        P_xz = outer_product_sum(
            self.inf*self.sigmas - self.inf*self.x, sigmas_h - z_mean) / (N - 1) 

        self.S = P_zz
        #self.SI = self.inv(self.S)
        #self.K = dot(P_xz, self.SI)
        self.K = np.linalg.solve(self.S.T, P_xz.T).T 
        e_r = self._rng.multivariate_normal(self._mean_z, R, N)
        ...
        if self.inf_a is not None:
            self.sigmas = (1-self.inf_a)*self.sigmas - self.inf_a*self.x_prior
    ...
    def predict_anaet(self, dt_interval):
        ...
        process noise on integration time-steps you must use the normal predict attribute. 
